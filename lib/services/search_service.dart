...
class SearchService {
  ...
  Future<Map<String, dynamic>> getKeywordSummary(String keyword) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
            'Failed to get keyword summary: ${response.statusCode}');
      }
    } catch (e) {
      throw Exception('Failed to connect to server: $e');
    }
  ...
  Future<Map<String, dynamic>> getMultipleKeywordSummaries(
      List<String> keywords) async {
    final uri = Uri.parse('$_baseUrl/keyword-summary')
        .replace(queryParameters: {'keywords': keywords});
    try {
      final response = await http.get(uri);
      if (response.statusCode == 200) {
        return jsonDecode(response.body) as Map<String, dynamic>;
